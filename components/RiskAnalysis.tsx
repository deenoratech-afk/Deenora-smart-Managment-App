...
import { AlertTriangle, ShieldCheck, Zap, Loader2, ChevronRight, User, GraduationCap, Clock, TrendingDown, Banknote } from 'lucide-react';
...
const RiskAnalysis: React.FC<RiskAnalysisProps> = ({ madrasahId, lang, onStudentClick }) => {
  ...
  const highRisks = risks.filter(r => r.dropout_risk === 'high' || r.late_risk === 'high' || r.result_risk === 'high' || r.fee_risk === 'high');
  const warningRisks = risks.filter(r => 
    (r.dropout_risk === 'warning' || r.late_risk === 'warning' || r.result_risk === 'warning' || r.fee_risk === 'warning') && 
    !(r.dropout_risk === 'high' || r.late_risk === 'high' || r.result_risk === 'high' || r.fee_risk === 'high')
  );
          ...
          >
            <div className="flex items-center gap-4 min-w-0 flex-1">
              <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${
                risk.dropout_risk === 'high' || risk.late_risk === 'high' || risk.result_risk === 'high' || risk.fee_risk === 'high'
              ...
              <div className="min-w-0">
                ...
                <div className="flex flex-wrap gap-1.5 mt-1">
                  ...
                  {risk.fee_risk !== 'safe' && (
                    <span className={`px-2 py-0.5 rounded-full text-[7px] font-black uppercase tracking-tighter flex items-center gap-1 ${
                      risk.fee_risk === 'high' ? 'bg-red-500 text-white' : 'bg-orange-400 text-white'
                    }`}>
                      <Banknote size={8} /> ফি বকেয়া
                    </span>
                  )}
                </div>
              </div>
