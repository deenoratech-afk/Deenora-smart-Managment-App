...
const Accounting: React.FC<AccountingProps> = ({ lang, madrasah, onBack, role }) => {
  ...
  useEffect(() => {
    if (selectedItems.length > 0 && selectedStudent) {
      const total = selectedStudent.fee_breakdown
        .filter((item: any) => selectedItems.includes(item.id))
        .reduce((sum: number, item: any) => sum + item.amount, 0);
      setCollectAmount(total.toString());
    }
  ...
  const handleCollectFee = async () => {
    ...
    setIsSaving(true);
    try {
      // আমরা সবসময় একটি এন্ট্রি তৈরি করবো যা ম্যানুয়াল বা আইটেম ভিত্তিক হতে পারে
      // যদি আইটেম সিলেক্ট করা থাকে, তবে ডেসক্রিপশনে সেগুলো লিখে দেবো
      const itemsToPay = selectedItems.length > 0 
        ? selectedStudent.fee_breakdown.filter((item: any) => selectedItems.includes(item.id))
        : [];

      const description = feeNote.trim() || (itemsToPay.length > 0 
        ? `Fees: ${itemsToPay.map((i: any) => i.name).join(', ')}` 
        : 'Manual Fee');
      ...
      const { error: feeErr } = await supabase.from('fees').insert({
        ...
        status: (amt + disc) >= selectedStudent.balance_due ? 'paid' : 'partial'
      });
      ...
      if (amt > 0) {
        await supabase.from('ledger').insert({
          ...
          description: `Fee payment for ${selectedStudent.student_name}${itemsToPay.length > 0 ? ' (' + itemsToPay.map((i: any) => i.name).join(', ') + ')' : ''}`,
                       ...
                       <div className="flex items-center gap-3.5 min-w-0">
                          ...
                          <div className="min-w-0">
                             ...
                             <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                                ...
                                {item.total_discount > 0 && <p className="text-[9px] text-orange-500 font-black uppercase">ছাড়: ৳{item.total_discount}</p>}
              ...
              <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 space-y-6 animate-in zoom-in-95 overflow-y-auto max-h-[80vh] shadow-2xl">
                  ...
                  <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 text-center">
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-4">
                          ...
                          <div className="bg-white p-2.5 rounded-2xl border border-slate-100 shadow-sm">
                            <p className="text-[7px] font-black text-slate-400 uppercase"> Disc </p>
                            <p className="font-black text-orange-600 text-sm">৳{selectedStudent.total_discount || 0}</p>
                          </div>
                          <div className="bg-white p-2.5 rounded-2xl border border-slate-100 shadow-sm">
                  ...
                  <div className="space-y-4">
                      <div className="space-y-2.5">
                          ...
                          <div className="space-y-2">
                            {selectedStudent.fee_breakdown.map((item: any) => {
                              const paidData = selectedStudent.paid_map && selectedStudent.paid_map[item.id];
                              const amountPaid = paidData ? (typeof paidData === 'object' ? (paidData.paid || 0) : paidData) : 0;
                              const discountGiven = paidData && typeof paidData === 'object' ? (paidData.discount || 0) : 0;
                              const isFullyPaid = (amountPaid + discountGiven) >= item.amount;
                              ...
                              return (
                                <div 
                                  ...
                                  className={`p-4 rounded-2xl border transition-all flex items-center justify-between cursor-pointer ${isFullyPaid ? 'bg-emerald-50 border-emerald-100 opacity-60' : selectedItems.includes(item.id) ? 'bg-blue-50 border-blue-200 ring-2 ring-blue-100' : 'bg-slate-50 border-slate-100 hover:border-slate-200'}`}
                                >
                                  <div className="flex items-center gap-3">
                                    <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${isFullyPaid ? 'bg-emerald-500 border-emerald-500 text-white' : selectedItems.includes(item.id) ? 'bg-blue-500 border-blue-500 text-white' : 'bg-white border-slate-200'}`}>
                                      {(isFullyPaid || selectedItems.includes(item.id)) && <CheckCircle2 size={14} />}
                                    </div>
                                    <div>
                                  ...
                                  <div className="text-right">
                                    ...
                                    {isFullyPaid ? (
                                      <p className="text-[8px] font-black text-emerald-500 uppercase">পরিশোধিত</p>
                                    ) : (amountPaid + discountGiven) > 0 ? (
                                      <p className="text-[8px] font-black text-orange-500 uppercase">বাকি: ৳{item.amount - (amountPaid + discountGiven)}</p>
                                    ) : null}
                                  </div>
                                </div>
                              );
                      ...
                      <div className="space-y-4 pt-4 border-t border-slate-100">
                          <div className="grid grid-cols-2 gap-3">
                              <div className="space-y-1.5">
                                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">জমা (Amount)</label>
                                  <div className="relative">
                                    <input type="number" className="w-full h-14 bg-slate-50 rounded-2xl px-10 font-black text-lg outline-none border-2 border-transparent focus:border-[#2563EB]/20" placeholder="0" value={collectAmount} onChange={(e) => setCollectAmount(e.target.value)} />
                                    <DollarSign className="absolute left-3 top-4 text-[#2563EB]" size={18}/>
                                  </div>
                              </div>
                              <div className="space-y-1.5">
                                  <label className="text-[10px] font-black text-orange-400 uppercase tracking-widest px-2">ডিসকাউন্ট (Discount)</label>
                                  <div className="relative">
                                    <input type="number" className="w-full h-14 bg-orange-50 rounded-2xl px-10 font-black text-lg outline-none border-2 border-transparent focus:border-orange-500/20 text-orange-600" placeholder="0" value={discountAmount} onChange={(e) => setDiscountAmount(e.target.value)} />
                                    <Tag className="absolute left-3 top-4 text-orange-400" size={18}/>
                                  </div>
                              </div>
                          </div>
                          <div className="space-y-1.5">
                              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">নোট (ঐচ্ছিক)</label>
                              <input type="text" className="w-full h-12 bg-slate-50 rounded-xl px-4 font-bold text-xs outline-none border border-slate-100" placeholder="ফি সম্পর্কে কিছু লিখুন..." value={feeNote} onChange={(e) => setFeeNote(e.target.value)} />
                          </div>
                      </div>

                      <button 
                        onClick={handleCollectFee} 
                        disabled={isSaving || (!collectAmount && !discountAmount)} 
                      ...
                      >
                          {isSaving ? <Loader2 className="animate-spin" /> : (
                            <><CheckCircle2 size={20}/> পেমেন্ট নিশ্চিত করুন</>
                          )}
                      </button>
                  </div>
              </div>
