...
import { GraduationCap, Plus, ChevronRight, BookOpen, Trophy, Save, X, Edit3, Trash2, Loader2, ArrowLeft, Calendar, LayoutGrid, CheckCircle2, FileText, Send, User, Hash, Star, AlertCircle, TrendingUp, DollarSign } from 'lucide-react';
...
const Exams: React.FC<ExamsProps> = ({ lang, madrasah, onBack, role }) => {
  ...
  const handleAddExam = async () => {
    ...
    if (!error && newExam && examFee) {
      await supabase.from('exam_fees').insert({
        madrasah_id: madrasah.id,
        exam_id: newExam.id,
        amount: parseFloat(examFee)
      });
    }
        ...
        <div className="fixed inset-0 bg-[#080A12]/60 backdrop-blur-xl z-[999] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 space-y-6 animate-in zoom-in-95">
             ...
             <div className="space-y-4">
                ...
                <div className="relative">
                    <input type="number" className="w-full h-14 bg-slate-50 rounded-2xl px-12 font-black text-sm outline-none border-2 border-transparent focus:border-[#2563EB]/20 transition-all" placeholder="পরীক্ষার ফি (ঐচ্ছিক)" value={examFee} onChange={(e) => setExamFee(e.target.value)} />
